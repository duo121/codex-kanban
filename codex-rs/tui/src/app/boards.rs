...
impl App {
    ...
    pub(crate) async fn open_current_board_session_picker(&mut self) {
        ...
        self.open_board_session_picker_for_board(board_id, /*preferred_thread_id*/ None)
            .await;
    }
    ...
    async fn open_board_session_picker_for_board(
        ...
        if sessions.is_empty() {
            if let Err(err) = self
                .create_bound_board_session_in_board(/*tui*/ None, board_id.clone())
                .await
            {
                self.chat_widget
    ...
    pub(crate) async fn bind_board(
        ...
        let sessions = registry
            ...
            .map_err(board_registry_report)?;
        if sessions.is_empty() {
            self.create_bound_board_session_in_board(Some(tui), board_id.clone())
                .await?;
        } else {
            ...
            if !self
                .switch_to_board_session(tui, board_id.clone(), target_thread_id)
                .await?
            {
                self.create_bound_board_session_in_board(Some(tui), board_id.clone())
                    .await?;
    ...
    pub(crate) async fn create_bound_board_session(&mut self, tui: &mut tui::Tui) -> Result<()> {
        let Some(board_id) = self.bound_board_id.clone() else {
            self.chat_widget.add_info_message(
                "This window is not bound to a board.".to_string(),
                Some("Use /kb to choose or create a board.".to_string()),
        ...
        self.create_bound_board_session_in_board(Some(tui), board_id)
            .await?;
        self.open_current_board_session_picker().await;
        Ok(())
    }
    ...
    pub(crate) async fn switch_to_board_session(
        ...
        let Some(available_thread_id) = self
            .ensure_board_thread_available(board_id.as_str(), thread_id)
            .await?
        else {
            return Ok(false);
        };
        self.select_agent_thread(tui, available_thread_id).await?;
    ...
    pub(crate) async fn remove_board_session(
        ...
        {
            let _ = self
                .switch_to_board_session(tui, board_id.clone(), next_thread_id)
                .await?;
        } else {
            self.create_bound_board_session_in_board(Some(tui), board_id.clone())
                .await?;
    ...
    pub(crate) async fn rename_board_session(
        ...
        let submitted = self
            .submit_op_to_thread(thread_id, Op::SetThreadName { name: name.clone() })
            .await;
        if submitted
        ...
        self.open_board_session_picker_for_board(board_id, Some(thread_id))
            .await;
        Ok(())
    }
    ...
    pub(crate) async fn register_live_thread(&mut self, new_thread: NewThread) -> Result<ThreadId> {
    ...
    pub(crate) async fn register_live_thread_with_session_configured(
        &mut self,
        thread_id: ThreadId,
        ...
        match self.server.start_thread(self.config.clone()).await {
            Ok(new_thread) => {
                ...
                let title_snapshot = recovered_board_session_title(
                    ...
                    missing_thread_id,
                    new_thread.session_configured.thread_name.clone(),
                    new_thread_id,
                );
                self.register_live_thread(new_thread).await?;
    ...
    async fn create_bound_board_session_in_board(
        ...
        match self.server.start_thread(self.config.clone()).await {
            Ok(new_thread) => {
                ...
                if let Some(tui) = tui {
                    let _ = self
                        .switch_to_board_session(tui, board_id.clone(), thread_id)
                        .await?;
                }
            }
            Err(err) => self
    ...
    async fn ensure_board_thread_available(
        ...
        let Some(path) = registry.find_rollout_path_by_id(thread_id).await else {
            return self
                .recover_missing_board_session(board_id, thread_id)
                .await;
        };
            ...
            .await
        {
            ...
            Err(err) => {
                ...
                self.chat_widget.add_error_message(format!(
                    "Failed to resume board session from {path_display}: {err}",
                ));
                Ok(None)
            }
        }
...
#[cfg(test)]
mod tests {
    use super::board_session_picker_selected_idx;
    use super::replacement_thread_id_after_removal;
    use codex_core::boards::BoardSession;
    use codex_core::boards::BoardSessionStatus;
    use codex_protocol::ThreadId;
    use pretty_assertions::assert_eq;

    fn session(thread_id: ThreadId) -> BoardSession {
        BoardSession {
            board_id: "board-1".to_string(),
            thread_id,
            title_snapshot: String::new(),
            sort_order: 0,
            status: BoardSessionStatus::Unknown,
            last_seen_event_idx: None,
            last_event_idx: None,
    ...
    #[test]
    fn removal_fallback_prefers_next_session() {
        let thread_a = ThreadId::new();
        let thread_b = ThreadId::new();
        let thread_c = ThreadId::new();
        let remaining = vec![session(thread_a), session(thread_c)];

        assert_eq!(
            replacement_thread_id_after_removal(Some(1), &remaining),
            Some(thread_c)
        );
        assert_eq!(
            replacement_thread_id_after_removal(Some(0), &remaining),
            Some(thread_a)
        );
        assert_ne!(thread_b, thread_c);
    }

    #[test]
    fn removal_fallback_uses_previous_when_last_session_was_removed() {
        let thread_a = ThreadId::new();
        let thread_b = ThreadId::new();
        let remaining = vec![session(thread_a), session(thread_b)];

        assert_eq!(
            replacement_thread_id_after_removal(Some(2), &remaining),
            Some(thread_b)
        );
    }

    #[test]
    fn picker_selection_prefers_explicit_thread_over_current_thread() {
        let thread_a = ThreadId::new();
        let thread_b = ThreadId::new();
        let sessions = vec![session(thread_a), session(thread_b)];

        assert_eq!(
            board_session_picker_selected_idx(&sessions, Some(thread_b), Some(thread_a)),
            Some(1)
        );
        assert_eq!(
            board_session_picker_selected_idx(&sessions, None, Some(thread_a)),
            Some(0)
        );
    }
}
